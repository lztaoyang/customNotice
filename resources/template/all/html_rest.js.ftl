	...
	*/
    var vuePage = new Vue({
        el: '#app',
        data: {
        	sourceUrl:"${appName}/${instanceName}",
            //是否显示编辑窗口
            editFormVisible: false,
            //是否显示详情窗口
            detailFormVisible: false,
            //查询参数
            searchParams: {<#list columns as column>${column.fieldName}:'',</#list>},
            //分页数据,其中包含分页信息与数据列表
            pageData: {},
            //当前操作的实体（编辑/新增）
            entity: {<#list columns as column>${column.fieldName}:'',</#list>},
            //表格中选中的行的集合
            selectDatas: [],
	    ...
	    <#list columns as column>
	    	...
	    	<#elseif column.javaType=='BigDecimal'>
	    		Vali.double()
	    	<#elseif column.type!='DATE' && column.type!='DATETIME' && column.javaType !='Boolean' && !column.useDicValue>
	    		Vali.utf8mb4Len(false,${column.size})
	    	</#if>
	    	],
	    </#list>
            }
